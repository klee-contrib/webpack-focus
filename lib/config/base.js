...
var _optimize = require('webpack/optimize');
...
var baseConfig = function baseConfig(environnement, definedVariables) {
    ...
    config.addPlugin(10, function () {
        return new _webpack.DefinePlugin(config.getDefinedVariables());
    });
    ...
    if (parsedEnv.DEV && parsedEnv.HOT_RELOAD) {
        config.addPlugin(30, new _webpack.HotModuleReplacementPlugin());
    }
    ...
    if (parsedEnv.MINIMIFY) {
        config.addPlugin(50, function (env) {
            return new _optimize.UglifyJsPlugin({
                compressor: {
                    screw_ie8: true,
                    warnings: false,
