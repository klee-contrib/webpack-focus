...
const envParser = env => {
    ...
    if (newEnv.HOT_RELOAD && newEnv.HOT_OUTPUT_PUBLIC_PATH) {
        newEnv.OUTPUT_PUBLIC_PATH = newEnv.HOT_OUTPUT_PUBLIC_PATH;
    } else if (newEnv.HOT_RELOAD) {
        newEnv.OUTPUT_PUBLIC_PATH = `${newEnv.DEV_SERVER_PROTOCOL}://${newEnv.DEV_SERVER_HOST}:${
            newEnv.DEV_SERVER_PORT
        }/${newEnv.DEV_SERVER_SUBDOMAIN}`;
    }
