...
const defaultConfig = (environnement, definedVariables) => {
    ...
    const localFocus = environnement.LOCAL_FOCUS ? JSON.parse(environnement.env.LOCAL_FOCUS) : false;
    ...
    config.addDefinedVariable('__API_ROOT__', JSON.stringify(process.env.API_ROOT ? process.env.API_ROOT : `${API_PROTOCOL}://${API_HOST}:${API_PORT}/${API_SUBDOMAIN}`));
