...
const baseConfig = (environnement, definedVariables) => {
    ...
    config.setFilename(
        parsedEnv.USE_VERSION
            ? parsedEnv.npm_package_name + "." + parsedEnv.npm_package_version
            : parsedEnv.npm_package_name
    );
    ...
    config.setOptimization({
        minimize: parsedEnv.MINIMIFY,
        bail: environnement === "production",
        minimizer:
            parsedEnv.MINIMIFY &&
            new TerserPlugin({
                terserOptions: {
    ...
    if (parsedEnv.USE_CACHE) {
        babelLoaders.push({
            loader: "cache-loader",
            options: {
                cacheIdentifier: `cache-loader:{version} {process.env.NODE_ENV} ${"" + parsedEnv.LEGACY_EXPORTS} ${"" +
                    parsedEnv.HOT_RELOAD} ${"" + parsedEnv.LEGACY_LODASH} ${parsedEnv.BROWERS}`
            }
        });
    }
